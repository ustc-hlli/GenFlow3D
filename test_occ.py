...
def training_process(args, logger, chk_dir):
    model = models.GenFlow_rec_seq_mask(args.npoint)
    ...
    if(args.dataset.lower() == 'av2'):
        eval_set = datasets.AV2(args.data_root,
                                     args.npoint,
                                     args.length_pre,
                                     args.length_fut,
...
def eval_model(model, loader, args):
    ...
    total_loss = 0
    total_seen = 0
    total_valid = np.zeros([args.length_pre + args.length_fut - 1])
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(loader, 0), total=len(loader)):
            pcs, feats, flows, masks, fut_pcs = data
            ...
            batch_valid = np.zeros([args.length_pre + args.length_fut - 1])
            for j in range(len(masks)): #time
                cur_mask_sum = torch.sum(masks[j].squeeze(2), dim=1) #[B]
                cur_valid = (cur_mask_sum > 0).sum()
                batch_valid[j] = cur_valid.cpu().data.numpy()
            
            pred_flows, fps_inds, tot_loss_difs = model(pcs, feats, flows, masks)
            loss = models.multiscalerecurrentloss_seq_mask(flows, masks, pred_flows, fps_inds, [0.02, 0.04, 0.08, 0.16], tot_loss_difs, args.weight_dist, args.weight_dif)
            metrics = utils.compute_metrics_mask(pred_flows[0], flows, masks)
            fut_metrics = utils.comput_fut_metrics_mask(pred_flows[0][args.length_pre-1:], pcs[-1], masks[args.length_pre-1:], fut_pcs)
            
            total_loss += loss.cpu().data * cur_bs
            total_seen += cur_bs
            total_valid += batch_valid
            for k in metrics.keys():
                total_metrics[k] += metrics[k] * batch_valid
            for k in fut_metrics.keys():
                total_metrics[k] += fut_metrics[k] * batch_valid[args.length_pre-1:]
        
        mean_loss = total_loss / total_seen
        for k in total_metrics.keys():
            if(k == 'chamfer'):
                total_metrics[k] /= total_valid[args.length_pre-1:]
            else:
                total_metrics[k] /= total_valid
    return mean_loss, total_metrics

if __name__ == '__main__':
    root = os.path.dirname(os.path.abspath(__file__))
    args = cmd_args.parse_args_from_yaml(os.path.join(root, 'test_cfg_occ.yaml'))
    ...
    files_to_save = ['models.py', 'units.py', 'layers.py', 'test_occ.py', 'test_cfg_occ.yaml', 'datasets.py', 'utils.py']
